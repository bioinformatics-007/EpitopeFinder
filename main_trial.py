...
def main():
    ...
    check_dependencies()
    while True:
        input_value = input("Enter the input (FASTA file path or UniProt ID): ").strip()
        ...
        try:
            if is_uniprot:
                ...
                if not uniprot_mapping:
                    logger.error(f"Failed to process UniProt ID: {input_value}")
                    print_status(f"Error: Failed to process UniProt ID: {input_value}", "error")
                    continue
            else:
                input_file = Path(input_value)
                if not input_file.is_file():
                    logger.error(f"Invalid file path: {input_file}")
                    print_status(f"Error: Invalid file path: {input_file}", "error")
                    continue
                if not validate_fasta_file(input_file):
                    logger.error("Invalid FASTA file")
                    print_status("Error: Invalid FASTA file. Please try again.", "error")
                    continue
                temp_dir = Path(tempfile.mkdtemp(prefix="epitopefinder_"))
                batch_files, batch_dirs, uniprot_mapping = split_fasta_into_batches(input_file, BATCH_SIZE, temp_dir)
                if not batch_files:
                    logger.error("No valid sequences found in FASTA file")
                    print_status("Error: No valid sequences found in FASTA file", "error")
                    continue
            
            selected_pathogen_type = select_pathogen_type()
            if selected_pathogen_type == "unknown":
                logger.error("Invalid pathogen type selection")
                print_status("Error: Invalid pathogen type selection. Please try again.", "error")
                continue
            ...
            # Print strategy names before the prompt
            ...
            strategy = input("Select strategy (1/2/3/4/5): ").strip()
            ...
            if strategy in strategies:
                if strategy == "1":
                    ...
                    print("Executing Strategy 1: Epitope Prediction")  # Fallback print
                    failed_tools = strategies[strategy](input_file, results_dir, confirmed_pathogen_type, logger, batch_files, batch_dirs, uniprot_mapping)
                elif strategy == "2":
                    print_status("Executing Strategy 2: Protein Prioritization", "info")
                    print("Executing Strategy 2: Protein Prioritization")  # Fallback print
                    failed_tools = strategies[strategy](input_file, results_dir, input_value, is_uniprot, logger, batch_files, batch_dirs, uniprot_mapping)
                elif strategy == "3":
                    print_status("Executing Strategy 3: Virulence, Toxicity, and Host Compatibility Assessment", "info")
                    print("Executing Strategy 3: Virulence, Toxicity, and Host Compatibility Assessment")  # Fallback print
                    failed_tools = strategies[strategy](input_file, confirmed_pathogen_type, results_dir, logger, batch_files, batch_dirs, uniprot_mapping)
                elif strategy == "4":
                    print_status("Executing Strategy 4: Comprehensive Multi-Tool Analysis", "info")
                    print("Executing Strategy 4: Comprehensive Multi-Tool Analysis")  # Fallback print
                    failed_tools = strategies[strategy](input_file, confirmed_pathogen_type, results_dir, logger, batch_files, batch_dirs, uniprot_mapping)
                elif strategy == "5":
                    print_status("Executing Strategy 5: Predicted Epitope Analysis", "info")
                    print("Executing Strategy 5: Predicted Epitope Analysis")  # Fallback print
                    failed_tools = strategies[strategy](input_file, confirmed_pathogen_type, results_dir, logger, batch_files, batch_dirs, uniprot_mapping)
                else:
                    logger.error(f"Unexpected strategy: {strategy}")
                    print_status(f"Error: Unexpected strategy: {strategy}", "error")
            else:
                logger.error(f"Invalid strategy selected: {strategy}")
                print_status("Error: Invalid strategy. Please choose 1, 2, 3, 4, or 5.", "error")
            if failed_tools:
                logger.warning(f"Failed tools: {', '.join(failed_tools)}")
                print_status(f"Warning: The following tools failed: {', '.join(failed_tools)}", "warning")
            while True:
                run_again = input("\nDo you want to run another strategy? (y/n): ").strip().lower()
                if run_again in ['y', 'n']:
                    break
            ...
            if run_again == 'n':
                ...
                # Print strategy names before the prompt in the run again loop
                print("\nAvailable strategies:")
                print("1 - Epitope Prediction")
                print("2 - Protein Prioritization")
                print("3 - Virulence, Toxicity, and Host Compatibility Assessment")
